...
pub struct WindowToEguiContextMap {
    ...
    pub window_to_contexts:
        bevy::platform::collections::HashMap<Entity, bevy::platform::collections::HashSet<Entity>>,
    /// Indexes windows by contexts.
    pub context_to_window: bevy::platform::collections::HashMap<Entity, Entity>,
}
...
pub fn process_ime_system(
    ...
    // NOTE: IME support disabled since we own winit directly
    // You can implement IME by accessing your winit window and calling:
    // - window.set_ime_allowed(bool)
    // - window.set_ime_cursor_area(position, size)
    // when egui_output.platform_output.ime is Some

    for (_entity, _egui_output, egui_settings, _egui_context, _egui_ime_state) in
...
pub fn custom_input_system(
    ...
    let mut current_pointer_pos = egui::Pos2::ZERO;
    ...
    for event in input_buffer.events.iter() {
        match event {
            InputEvent::MouseMove { pos } => {
                ...
                for (_entity, _settings, mut pointer_pos) in egui_contexts.iter_mut() {
                    pointer_pos.position = current_pointer_pos;
                }

                // Send PointerMoved to egui for hover tracking
                for (entity, _settings, _pointer_pos) in egui_contexts.iter() {
                    egui_input_message_writer.write(EguiInputEvent {
                        context: entity,
                        event: egui::Event::PointerMoved(current_pointer_pos),
                    });
                    messages_written += 1;
                }
            }
                    ...
                    TouchPhase::Started => {
                        ...
                        for (entity, _settings, pointer_pos) in egui_contexts.iter() {
                            egui_input_message_writer.write(EguiInputEvent {
                                context: entity,
                                event: egui::Event::PointerButton {
                                    pos: pointer_pos.position,
                                    button: egui_button,
                                    pressed: true,
                                    modifiers: modifier_keys_state.to_egui_modifiers(),
                                },
                            });
                            messages_written += 1;
                        }
                ...
                for (entity, settings, _pointer_pos) in egui_contexts.iter() {
                    ...
                    egui_input_message_writer.write(EguiInputEvent {
                        ...
                        event: egui::Event::MouseWheel {
                            ...
                            delta: egui::vec2(delta.x / scale_factor, delta.y / scale_factor),
                            modifiers: modifier_keys_state.to_egui_modifiers(),
                        },
                    });
                    messages_written += 1;
            ...
            InputEvent::Keyboard {
                ...
                modifier_keys_state.shift = modifiers.shift;
                modifier_keys_state.ctrl = modifiers.ctrl;
                modifier_keys_state.alt = modifiers.alt;
                modifier_keys_state.win = modifiers.meta;

                // Convert engine KeyCode to egui Key
                if let Some(egui_key) = engine_keycode_to_egui(*key) {
                    for (entity, _settings, _pointer_pos) in egui_contexts.iter() {
                        egui_input_message_writer.write(EguiInputEvent {
                            context: entity,
                            event: egui::Event::Key {
                                key: egui_key,
                                physical_key: None, // We don't track physical keys separately
                                pressed: *pressed,
                                repeat: false, // We don't track key repeats
                                modifiers: modifier_keys_state.to_egui_modifiers(),
                            },
                        });
                        messages_written += 1;
