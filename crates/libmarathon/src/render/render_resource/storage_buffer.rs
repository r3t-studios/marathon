...
use encase::{
    DynamicStorageBuffer as DynamicStorageBufferWrapper,
    ShaderType,
...
/// storage buffer.
///
/// Storage buffers can be made available to shaders in some combination of
/// read/write mode, and can store large amounts of data. Note however that
/// WebGL2 does not support storage buffers, so consider alternative options in
/// this case.
///
/// Storage buffers can store runtime-sized arrays, but only if they are the
/// last field in a structure.
///
/// The contained data is stored in system RAM.
/// [`write_buffer`](StorageBuffer::write_buffer) queues copying of the data
/// from system RAM to VRAM. Storage buffers must conform to [std430
...
///
/// This is just a [`StorageBuffer`], but also allows you to set dynamic
/// offsets.
///
/// Dynamic storage buffers can be made available to shaders in some combination
/// of read/write mode, and can store large amounts of data. Note however that
/// WebGL2 does not support storage buffers, so consider alternative options in
/// this case. Dynamic storage buffers support multiple separate bindings at
/// dynamic byte offsets and so have a [`push`](DynamicStorageBuffer::push)
/// method.
///
/// The contained data is stored in system RAM.
/// [`write_buffer`](DynamicStorageBuffer::write_buffer) queues copying of the
/// data from system RAM to VRAM. The data within a storage buffer binding must
/// conform to [std430 alignment/padding requirements]. `DynamicStorageBuffer`
/// takes care of serializing the inner type to conform to these requirements.
/// Each item [`push`](DynamicStorageBuffer::push)ed into this structure
