...
/// A subset of the `Material` trait for defining extensions to a base
/// `Material`, such as the builtin `StandardMaterial`.
///
/// A user type implementing the trait should be used as the `E` generic param
/// in an `ExtendedMaterial` struct.
pub trait MaterialExtension: Asset + AsBindGroup + Clone + Sized {
    /// Returns this material's vertex shader. If [`ShaderRef::Default`] is
    /// returned, the base material mesh vertex shader will be used.
    fn vertex_shader() -> ShaderRef {
        ShaderRef::Default
    }

    /// Returns this material's fragment shader. If [`ShaderRef::Default`] is
    /// returned, the base material mesh fragment shader will be used.
    fn fragment_shader() -> ShaderRef {
        ShaderRef::Default
    }

    // Returns this material’s AlphaMode. If None is returned, the base material
    // alpha mode will be used.
    fn alpha_mode() -> Option<AlphaMode> {
        None
    }

    /// Returns this material's prepass vertex shader. If [`ShaderRef::Default`]
    /// is returned, the base material prepass vertex shader will be used.
    fn prepass_vertex_shader() -> ShaderRef {
        ShaderRef::Default
    }

    /// Returns this material's prepass fragment shader. If
    /// [`ShaderRef::Default`] is returned, the base material prepass fragment
    /// shader will be used.
    ...
    /// [`ShaderRef::Default`] is returned, the base material deferred vertex
    /// shader will be used.
    ...
    /// Returns this material's prepass fragment shader. If
    /// [`ShaderRef::Default`] is returned, the base material deferred fragment
    /// shader will be used.
    ...
    /// [`MeshVertexBufferLayoutRef`] as input. Specialization for the base
    /// material is applied before this function is called.
    #[expect(
        unused_variables,
...
/// A material that extends a base [`Material`] with additional shaders and
/// data.
///
/// The data from both materials will be combined and made available to the
/// shader so that shader functions built for the base material (and referencing
/// the base material bindings) will work as expected, and custom alterations
/// based on custom data can also be used.
///
/// If the extension `E` returns a non-default result from `vertex_shader()` it
/// will be used in place of the base material's vertex shader.
///
/// If the extension `E` returns a non-default result from `fragment_shader()`
/// it will be used in place of the base fragment shader.
///
/// When used with `StandardMaterial` as the base, all the standard material
/// fields are present, so the `pbr_fragment` shader functions can be called
/// from the extension shader (see the `extended_material` example).
...
// We don't use the `TypePath` derive here due to a bug where
// `#[reflect(type_path = false)]` causes the `TypePath` derive to not generate
// an implementation.
