//! Render high-poly 3d meshes using an efficient GPU-driven method. See
//! [`MeshletPlugin`] and [`MeshletMesh`] for details.
...
use self::{
    ...
    material_shade_nodes::{
        MeshletDeferredGBufferPrepassNode,
        MeshletMainOpaquePass3dNode,
        MeshletPrepassNode,
    },
    ...
    resource_manager::{
        ResourceManager,
        prepare_meshlet_per_frame_resources,
        prepare_meshlet_view_bind_groups,
    },
...
use crate::render::{
    ...
    pbr::{
        ...
        graph::NodePbr,
        meshlet::meshlet_mesh_manager::init_meshlet_mesh_manager,
    },
    prepass::{
        DeferredPrepass,
        MotionVectorPrepass,
...
/// Provides a plugin for rendering large amounts of high-poly 3d meshes using
/// an efficient GPU-driven method. See also [`MeshletMesh`].
///
/// Rendering dense scenes made of high-poly meshes with thousands or millions
/// of triangles is extremely expensive in Bevy's standard renderer. Once meshes
/// are pre-processed into a [`MeshletMesh`], this plugin can render these kinds
/// of scenes very efficiently.
///
/// In comparison to Bevy's standard renderer:
/// * Much more efficient culling. Meshlets can be culled individually, instead
///   of all or nothing culling for entire meshes at a time. Additionally,
///   occlusion culling can eliminate meshlets that would cause overdraw.
/// * Much more efficient batching. All geometry can be rasterized in a single
///   draw.
/// * Scales better with large amounts of dense geometry and overdraw. Bevy's
///   standard renderer will bottleneck sooner.
/// * Near-seamless level of detail (LOD).
/// * Much greater base overhead. Rendering will be slower and use more memory
///   than Bevy's standard renderer with small amounts of geometry and overdraw.
/// * Requires preprocessing meshes. See [`MeshletMesh`] for details.
/// * Limitations on the kinds of materials you can use. See [`MeshletMesh`] for
///   details.
///
/// This plugin requires a fairly recent GPU that supports
/// [`WgpuFeatures::TEXTURE_INT64_ATOMIC`].
///
/// This plugin currently works only on the Vulkan and Metal backends.
///
/// This plugin is not compatible with [`Msaa`]. Any camera rendering a
/// [`MeshletMesh`] must have [`Msaa`] set to [`Msaa::Off`].
///
/// Mixing forward+prepass and deferred rendering for opaque materials is not
/// currently supported when using this plugin. You must use one or the other by
/// setting [`crate::DefaultOpaqueRendererMethod`]. Do not override
/// [`crate::Material::opaque_render_method`] for any material when using this
/// plugin.
///
...
pub struct MeshletPlugin {
    ...
    ///
    /// If this number is too low, you'll see rendering artifacts like missing
    /// or blinking meshes.
    ///
...
fn configure_meshlet_views(
    ...
    for (entity, msaa, normal_prepass, motion_vector_prepass, deferred_prepass) in &mut views_3d {
        ...
        if !(normal_prepass || motion_vector_prepass || deferred_prepass) {
            ...
            // TODO: Should we add both Prepass and DeferredGBufferPrepass materials here,
            // and in other systems/nodes?
