...
impl<T: PersistentGpuBufferable> PersistentGpuBuffer<T> {
    ...
    /// Queue an item of type T to be added to the buffer, returning the byte
    /// range within the buffer that it will be located at.
...
pub trait PersistentGpuBufferable {
    /// Additional metadata associated with each item, made available during
    /// `write_bytes_le`.
    type Metadata;

    /// The size in bytes of `self`. This will determine the size of the buffer
    /// passed into `write_bytes_le`.
    ///
    /// All data written must be in a multiple of `wgpu::COPY_BUFFER_ALIGNMENT`
    /// bytes. Failure to do so will result in a panic when using
    /// [`PersistentGpuBuffer`].
    fn size_in_bytes(&self) -> usize;

    /// Convert `self` + `metadata` into bytes (little-endian), and write to the
    /// provided buffer slice. Any bytes not written to in the slice will be
    /// zeroed out when uploaded to the GPU.
    fn write_bytes_le(
        &self,
