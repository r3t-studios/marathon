...
/// and may be of two kinds: [`NodeEdge`](Self::NodeEdge) and
/// [`SlotEdge`](Self::SlotEdge).
///
/// Edges are added via the [`RenderGraph::add_node_edge`] and the
/// [`RenderGraph::add_slot_edge`] methods.
///
/// The former simply states that the `output_node` has to be run before the
/// `input_node`, while the later connects an output slot of the `output_node`
...
pub enum Edge {
    /// An edge describing to ordering of both nodes (`output_node` before
    /// `input_node`) and connecting the output slot at the `output_index`
    /// of the `output_node` with the slot at the `input_index` of the
    /// `input_node`.
    SlotEdge {
