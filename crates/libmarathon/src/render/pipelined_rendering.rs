...
/// A Label for the sub app that runs the parts of pipelined rendering that need
/// to run on the main thread.
///
/// The Main schedule of this app can be used to run logic after the render
/// schedule starts, but before I/O processing. This can be useful for something
/// like frame pacing.
...
/// The [`PipelinedRenderingPlugin`] can be added to your application to enable
/// pipelined rendering.
///
/// This moves rendering into a different thread, so that the Nth frame's
/// rendering can be run at the same time as the N + 1 frame's simulation.
///
/// ```text
/// |--------------------|--------------------|--------------------|--------------------|
...
/// ```
///
/// The plugin is dependent on the [`RenderApp`] added by
/// [`crate::RenderPlugin`] and so must be added after that plugin. If it is not
/// added after, the plugin will do nothing.
///
...
/// ```
///
/// - `sync` is the step where the entity-entity mapping between the main and
///   render world is updated. This is run on the main app's thread. For more
///   information checkout [`SyncWorldPlugin`].
/// - `extract` is the step where data is copied from the main world to the
///   render world. This is run on the main app's thread.
/// - On the render thread, we first apply the `extract commands`. This is not
///   run during extract, so the main schedule can start sooner.
/// - Then the `rendering schedule` is run. See
///   [`RenderSystems`](crate::RenderSystems) for the standard steps in this
///   process.
/// - In parallel to the rendering thread the [`RenderExtractApp`] schedule
///   runs. By default, this schedule is empty. But it is useful if you need
///   something to run before I/O processing.
/// - Next all the `winit events` are processed.
/// - And finally the `main app schedule` is run.
/// - Once both the `main app schedule` and the `render schedule` are finished
///   running, `extract` is run again.
///
...
impl Plugin for PipelinedRenderingPlugin {
    ...
    // Sets up the render thread and inserts resources into the main app used for
    // controlling the render thread.
    fn cleanup(&self, app: &mut App) {
        ...
        std::thread::spawn(move || {
            ...
            loop {
                // run a scope here to allow main world to use this thread while it's waiting
                // for the render app
...
fn renderer_extract(app_world: &mut World, _world: &mut World) {
    app_world.resource_scope(|world, main_thread_executor: Mut<MainThreadExecutor>| {
        world.resource_scope(|world, mut render_channels: Mut<RenderAppChannels>| {
            // we use a scope here to run any main thread tasks that the render world still
            // needs to run while we wait for the render world to be received.
