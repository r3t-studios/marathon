...
///
/// **Security Note**: This is currently debug-only. See issue #135 for
/// production security requirements (authentication, rate limiting, etc.).
///
/// # Platform Support
///
            ...
            match listener.accept().await {
                | Ok((mut stream, _addr)) => {
                    ...
                    tokio::spawn(async move {
                        ...
                        // Execute command
                        let response =
                            handle_control_command(command, &bridge, session_id, node_id).await;
...
async fn handle_control_command(
    ...
    match command {
        ...
        | ControlCommand::InjectMessage { message } => match bridge.push_incoming(message) {
            | Ok(_) => ControlResponse::Ok {
        ...
        | ControlCommand::JoinSession { session_code } => ControlResponse::Error {
            error: format!(
                "JoinSession not yet implemented (requested: {})",
                session_code
            ),
        },
        | ControlCommand::LeaveSession => ControlResponse::Error {
            error: "LeaveSession not yet implemented".to_string(),
        },
        | ControlCommand::GetSessionInfo => ControlResponse::Error {
            error: "GetSessionInfo not yet implemented".to_string(),
        },
        | ControlCommand::ListSessions => ControlResponse::Error {
            error: "ListSessions not yet implemented".to_string(),
        },
        | ControlCommand::DeleteSession { session_code } => ControlResponse::Error {
            error: format!(
                "DeleteSession not yet implemented (requested: {})",
                session_code
            ),
        },
        | ControlCommand::ListPeers => ControlResponse::Error {
            error: "ListPeers not yet implemented".to_string(),
        },
        | ControlCommand::SpawnEntity { .. } => ControlResponse::Error {
            error: "SpawnEntity not available on session-level socket. Use app-level socket."
                .to_string(),
        },
        | ControlCommand::DeleteEntity { .. } => ControlResponse::Error {
            error: "DeleteEntity not available on session-level socket. Use app-level socket."
                .to_string(),
        },
    }
}
...
pub fn spawn_control_socket(_session_id: SessionId, _bridge: GossipBridge, _node_id: Uuid) {
    // TODO(#135): Implement secure control socket for release builds with
    // authentication
}
